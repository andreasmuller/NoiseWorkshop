...
#pragma include "Shaders/Common/PhotoshopBlendModes.glslinc"
...
uniform float u_meshStartScale;
uniform float u_meshEndScale;
...
void main ()
{
	...
	//v_particleColor.rgb = hsv2rgb( vec3( atan(particleVel.y, particleVel.x), 1.0, 1.0) );
	
	// scale by particle speed
	//float tmpVelFrac = clamp( length(particleVel) / 5.0, 0.1, 1.0 );
	//vertexPos.z *= tmpVelFrac;

	float meshScale =  mix(u_meshStartScale, u_meshEndScale, ageFrac );
	...
	vertexPos *= meshScale * smoothStepInOut( 0.0, 0.05, 0.95, 1.0, ageFrac );
