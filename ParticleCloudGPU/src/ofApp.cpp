...
void ofApp::setup()
{
	...
	particleDataFbo.dest()->getTextureReference(0).loadData( (float*)&startPositionsAndAge[0].x,	 textureSize, textureSize, GL_RGBA );
...
void ofApp::updateParticles()
{
	...
	particleDataFbo.dest()->begin();
		...
		particleUpdate.begin();
			...
			particleUpdate.setUniformTexture( "positionAndAgeTex",	particleDataFbo.source()->getTextureReference(0), 0 );
			//particleUpdate.setUniformTexture( "velocityTex",		particleDataFbo.source()->getTextureReference(1), 1 );
			
			particleUpdate.setUniform1f("time", time );
			particleUpdate.setUniform1f("timeStep", timeStep );
			...
			particleUpdate.setUniform3f("baseSpeed", baseSpeed.get().x, baseSpeed.get().y, baseSpeed.get().z );
...
void ofApp::drawParticles()
{
	...
	particleDrawUnsorted.begin();
		...
		particleDrawUnsorted.setUniformTexture("u_particleTexture", particleDataFbo.source()->getTextureReference(), 0 );
		
		particleDrawUnsorted.setUniformMatrix4f("u_viewMatrix", camera.getModelViewMatrix() );
		particleDrawUnsorted.setUniformMatrix4f("u_projectionMatrix", camera.getProjectionMatrix() );
		particleDrawUnsorted.setUniformMatrix4f("u_modelViewProjectionMatrix", camera.getModelViewProjectionMatrix() );
...
void ofApp::draw()
{
	...
	camera.begin();
		...
		drawParticles(); // this can be outside of the camera.begin() and .end() as we'll supply our own camera matrices
	...
	int size = 196;
	//particleDataFbo.source()->getTextureReference(0).draw( 0,	 0, size, size );
